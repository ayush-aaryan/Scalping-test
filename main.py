...
ALPACA_API_KEY = "PK2POJ7WYZBQ7QKGHQM7"
ALPACA_SECRET_KEY = "1VYySJzdVezCQieoYDjKQyurshSlnBMxgiJwNkym"
...
class ScalpAlgo:
    ...
    def _calc_buy_signal(self):
        ...
        if (fema>sema) and macd_line > signal_line:
            self._l.info(
    ...
    def on_bar(self, bar):
            self._bars = self._bars.append(pd.DataFrame({
                ...
                'high': bar.high,
                'low': bar.low,
                'close': (bar.close + bar.open + bar.high + bar.low)/4,
                'volume': bar.volume,
    ...
    def _submit_sell(self, bailout=False):
        ...
        if bailout:
            ...
            limit_price = max(cost_basis * 1.002, current_price)
            params.update(dict(
                type='limit',
